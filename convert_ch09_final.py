#!/usr/bin/env python3
"""
Complete LaTeX to PreTeXt XML converter for Chapter 9 - Multiple and logistic regression
        ...
        while i < len(lines):
            ...
            if '\\Figure[' in stripped:
                ...
                if match:
                    ...
                    self.output.append(f'{self.get_indent()}  <image source="ch_regr_mult_and_log/figures/{ref}"/>')
            ...
            if '\\FigureFullPath[' in stripped:
                ...
                if match:
                    ...
                    self.output.append(f'{self.get_indent()}  <image source="ch_regr_mult_and_log/figures/{path}"/>')
                    self.output.append(f'{self.get_indent()}</figure>')
                    self.output.append('')
                i += 1
                continue
            ...
            if stripped.startswith('\\begin{figure}'):
                ...
                if label or image_ref:
                    ...
                    if caption:
                        self.output.append(f'{self.get_indent()}  <caption>{caption}</caption>')
                    if image_ref:
                        self.output.append(f'{self.get_indent()}  <image source="ch_regr_mult_and_log/figures/{image_ref}"/>')
                    self.output.append(f'{self.get_indent()}</figure>')
                    self.output.append('')
                    self.indent_level += 1
...
def main():
    """Main entry point"""
    base_path = 'ch_regr_mult_and_log/TeX'
    input_file = os.path.join(base_path, 'ch_regr_mult_and_log.tex')
